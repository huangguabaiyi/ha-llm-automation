...
### YAML 注释规范（用 # 加注释，帮助理解逻辑）：
  - `description:` 字段用一句英文说明整体用途（必填，即使留空也要写 description: ""）
  - triggers 块前加 `# 触发条件：...` 中文注释
  - conditions 块前加 `# 条件检查：...` 中文注释（如无条件则省略）
  - actions 每个主要步骤前加 `# 执行动作：...` 中文注释
  - 复杂的条件分支逻辑必须注释说明意图
