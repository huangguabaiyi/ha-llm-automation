...
### Step 1：为每条自动化识别「使用场景」
常见 HA 使用场景（不限于此）：
- **离家节能**：人离开后关灯、关电扇、关空调、关电视、关窗帘等
- **到家迎接**：人回家后开灯、开空调、播放音乐等
- **睡前准备**：睡觉前关灯、降低亮度、关窗帘、关空调或调低温度等
- **起床唤醒**：早上起床时开灯、升高亮度、开窗帘等
- **夜间安全**：夜间运动检测触发补光、警报等
- **定时控制**：固定时间点对多个设备执行操作
- **温湿度联动**：根据温度/湿度控制空调/加湿器等

### Step 2：判断合并条件（以场景为唯一依据）
**核心原则：场景相同才是合并的依据，操作不同的设备、触发时间不完全一致，都不妨碍合并。**

✅ 应该合并：
- 属于同一使用场景（离家节能/到家迎接/睡前准备等），无论触发时间是否一致
  - 例：「离家后关灯」+「离家后关电扇」→ 同为离家节能，合并
  - 例：「每天22点关客厅灯」+「每天22:30关卧室灯」→ 同为睡前关灯场景，合并
  - 例：「到家开客厅灯」+「到家开空调」→ 同为到家迎接，合并
- 触发条件相同（或功能等价），且动作互补，属于同一目的

❌ 不应该合并：
- 场景目的明显不同（如节能 vs 安防 vs 温湿度联动）
- 触发类型根本不同（如一个按时间定时、一个按人员回家状态），且不同场景
- 一条是场景主流程，另一条有明显额外的特殊条件分支，合并后逻辑会变复杂

【强制要求】：
- 每条自动化必须且只能出现在 merge_groups、fix_items、ok_items 三者之一中
- 不得遗漏任何一条自动化，即使只有一条也要逐条处理
- 独立且无问题的条目必须放入 ok_items（不得静默忽略）
- 即使只有两条自动化，也要逐条检查是否有 fix_items 问题（幻觉实体、旧字段名、缺少 description 等）

### Step 3：找出需要修复的问题
- 引用了不存在的实体 ID（对照上方实体列表验证）
- 字段格式错误：用了旧版 trigger:/action:/service: 而非 triggers:/actions:
- 缺少 description 字段，或 description 为空/过短
- 逻辑明显有误（如 condition 永远不满足）

### Step 4：生成合并后的完整 YAML
- 保留所有被合并自动化涉及的**全部**实体和操作，一个都不能遗漏
- 统一使用 HA 2024+ 字段格式（triggers/conditions/actions）
- description 用完整英文句子描述触发时机和所有执行动作
...
- 【全量覆盖】每条自动化必须出现在 merge_groups（被合并）、fix_items（需修复）、ok_items（无需改动）三者之一，不允许遗漏任何一条
...
- 如无需合并/修复，对应列表返回空数组 []，但 ok_items 必须列出所有未被合并/修复的自动化\
"""
