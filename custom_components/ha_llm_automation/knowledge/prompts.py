...
### Step 2：判断合并条件（满足以下任一即可合并）
**核心原则：操作不同的设备不是不能合并的理由，场景相同才是合并的依据。**

✅ 应该合并：
- 触发条件相同（或功能等价），且都属于同一使用场景
  - 例：「离家后关灯」+「离家后关电扇」→ 同为离家节能，合并为一条含所有设备的自动化
  - 例：「每天22点关客厅灯」+「每天22点关卧室灯」→ 同为定时关灯，合并
  - 例：「到家开客厅灯」+「到家开空调」→ 同为到家迎接，合并
- 触发条件高度重叠（时间差在5分钟内），且属于同一场景

❌ 不应该合并：
- 触发条件不同（如一个按时间、一个按人员状态），即使涉及同类设备
- 场景目的明显不同（如节能 vs 安防）
- 一条是场景主流程，另一条有额外的特殊条件分支
